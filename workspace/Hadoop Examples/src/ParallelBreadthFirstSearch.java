...
public class ParallelBreadthFirstSearch {
	...
	public static class Map extends Mapper<LongWritable, Text, IntWritable, Text> {
		...
		public void map(LongWritable key, Text value, Context context) throws IOException, InterruptedException {
			...
			for (int i = 0; i < nodes.length; i++) {
				NodeBFS node = new NodeBFS(nodes[i]);
				...
				if (sourceDistance >= 0) { //non traversed node
					Iterator<NodeBFS> adjacencyListIterator = node.getAdjacencyList().iterator();
					while (adjacencyListIterator.hasNext()) {
						NodeBFS neighbour = adjacencyListIterator.next();
	...
	public static class Reduce extends Reducer<IntWritable, Text, IntWritable, Text> {
		...
		public void reduce(IntWritable key, Iterable<Text> values, Context context) throws IOException, InterruptedException {
			...
			NodeBFS destinationNode = null;
			for (Text value : values) {
				NodeBFS node = new NodeBFS(key.get() + "  " + value.toString());
